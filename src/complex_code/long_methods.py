...
def process_customer_order_long(order_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for update in inventory_updates:
        ...
        try:
            ...
            result["messages"].append(f"Warning: Inventory update failed for product {product_id}")
    ...
    try:
        if customer_data.get("email"):
            ...
            for item in items_with_details:
                email_body += f"- {item['name']} x {item['quantity']} - ${item['total']:.2f}\n"
    ...
    try:
        ...
        # Update total spent
        customer_data["total_spent"] = customer_data.get("total_spent", 0) + result["total"]
